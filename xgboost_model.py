...
#####################
#model = xgb.XGBClassifier(max_depth=7, n_estimators=1000, learning_rate=0.05).fit(X_train, Y_train)
#####################
...
#grid_search = GridSearchCV(model, param_grid, scoring="f1", n_jobs=-1, cv=kfold, verbose=10)
...
compare.to_csv("compare_xgb_logloss.csv")

compare1=pd.DataFrame(dt.iloc[0:42000,0])
compare1['model']=model.predict_proba(X_test)[:,1]
compare1.to_csv("compare_xgb_logloss_prob.csv")
